/**
 * CookingModePage.tsx v2 — Timer apre Google, Wake Lock API, testo grande.
 */
...
import { extractTimersFromStep } from "../hooks/useScaledIngredients";
    ...
    <div className="modal-overlay" onClick={onCancel}>
      <div className="modal" onClick={(e) => e.stopPropagation()} style={{ textAlign: "center" }}>
        ...
        <p style={{ color: "var(--text-muted)", marginBottom: "1.5rem", fontSize: "0.9rem" }}>
          Le quantità verranno scalate automaticamente
        </p>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "1rem", marginBottom: "0.75rem" }}>
          <button onClick={() => setN((v) => clampServings(v - 1))}
            style={{ width: 44, height: 44, borderRadius: "50%", border: "2px solid var(--border)", background: "var(--bg-card)", fontSize: "1.5rem", cursor: "pointer" }}>−</button>
...
function DoneScreen({ title, onClose }: { title: string; onClose: () => void }) {
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "2rem", textAlign: "center" }}>
      ...
      <p style={{ color: "var(--cooking-muted)", fontSize: "1.1rem", marginBottom: "2.5rem" }}>{title} completata ✓</p>
      <button onClick={onClose}
        style={{ background: "var(--cooking-brand)", color: "#fff", border: "none", borderRadius: "var(--radius-lg)", padding: "1rem 2rem", fontSize: "1.1rem", fontWeight: 700, cursor: "pointer", minWidth: 180 }}>
        Chiudi
      </button>
    </div>
  );
...
export default function CookingModePage() {
  ...
  const [fontSize, setFontSize] = useState(1); // moltiplicatore testo

  useWakeLock();

  const scaled = useScaledIngredients(recipe?.ingredients ?? [], recipe?.yield ?? 4, servings);
    ...
    return (
      <div style={{ minHeight: "100dvh", background: "var(--cooking-bg)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: "var(--cooking-muted)", textAlign: "center" }}>
          <div style={{ width: 40, height: 40, border: "3px solid rgba(255,255,255,0.2)", borderTopColor: "var(--cooking-brand)", borderRadius: "50%", margin: "0 auto 1rem", animation: "spin 0.8s linear infinite" }} />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="cooking-mode" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
      {phase === "modal" && (
        <ServingsModal defaultServings={qsServings ?? recipe.yield}
          onStart={(n) => { setServings(n); setPhase("cooking"); }}
          onCancel={() => navigate(-1)} />
      )}

      {phase === "done" && <DoneScreen title={recipe.title} onClose={() => navigate("/")} />}
      ...
      {phase === "cooking" && (
        <>
          ...
          <div style={{ background: "var(--cooking-surface)", padding: "0.75rem 1rem", paddingTop: "calc(0.75rem + env(safe-area-inset-top))", display: "flex", alignItems: "center", gap: "0.75rem" }}>
            ...
            <div style={{ flex: 1 }}>
              <p style={{ margin: 0, fontSize: "0.75rem", color: "var(--cooking-muted)", fontWeight: 700, letterSpacing: "0.05em" }}>
            ...
            <div style={{ display: "flex", gap: "0.25rem" }}>
              <button onClick={() => setFontSize((f) => Math.max(0.85, f - 0.15))}
                style={{ background: "rgba(255,255,255,0.1)", border: "none", borderRadius: 6, color: "var(--cooking-muted)", cursor: "pointer", padding: "0.3rem 0.5rem", fontSize: "0.9rem" }}>A−</button>
              <button onClick={() => setFontSize((f) => Math.min(1.6, f + 0.15))}
                style={{ background: "rgba(255,255,255,0.1)", border: "none", borderRadius: 6, color: "var(--cooking-muted)", cursor: "pointer", padding: "0.3rem 0.5rem", fontSize: "0.9rem" }}>A+</button>
            </div>
          </div>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "1.75rem 1.25rem", gap: "1.25rem", overflowY: "auto" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
              <span style={{ width: 40, height: 40, borderRadius: "50%", background: "var(--cooking-brand)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-serif)", fontWeight: 700, fontSize: "1.1rem", color: "#fff", flexShrink: 0 }}>
                {step + 1}
              </span>
              <span style={{ fontSize: "0.875rem", color: "var(--cooking-muted)" }}>
                {step < totalSteps - 1 ? `ancora ${totalSteps - step - 1} ${totalSteps - step - 1 === 1 ? "passo" : "passi"}` : "ultimo passo"}
              </span>
            </div>

            <p style={{ fontSize: `${fontSize * 1.4}rem`, lineHeight: 1.55, color: "var(--cooking-text)", flex: 1, margin: 0 }}>
              {currentStepText}
            </p>
            ...
            {stepTimers.length > 0 && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.625rem" }}>
                {stepTimers.map((t, i) => (
                  <TimerButton key={i} label={t.label} durationSeconds={t.seconds} />
                ))}
              </div>
            )}

            {/* Ingredienti (solo passo 0) */}
                ...
                <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: "0.35rem" }}>
                  {scaled.map((ing) => (
                    <li key={ing.id} style={{ fontSize: `${fontSize * 0.9}rem`, color: "var(--cooking-text)", display: "flex", gap: "0.5rem" }}>
                      <span style={{ fontWeight: 700, color: "var(--cooking-brand)", minWidth: 60, textAlign: "right" }}>
          ...
          {/* Nav buttons */}
          <div style={{
            background: "var(--cooking-surface)", padding: "1rem 1.25rem",
            paddingBottom: "calc(1rem + env(safe-area-inset-bottom))",
            display: "grid", gridTemplateColumns: step > 0 ? "1fr 1fr" : "1fr", gap: "0.625rem",
          }}>
            {step > 0 && (
              <button onClick={goPrev}
                style={{ minHeight: 60, fontSize: "1.1rem", borderRadius: "var(--radius-lg)", fontWeight: 700, background: "rgba(255,255,255,0.08)", color: "var(--cooking-muted)", border: "none", cursor: "pointer" }}>
                ← Indietro
              </button>
            )}
            <button onClick={goNext}
              style={{ minHeight: 60, fontSize: "1.1rem", borderRadius: "var(--radius-lg)", fontWeight: 700, background: "var(--cooking-brand)", color: "#fff", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }}>
              {step === totalSteps - 1 ? <><IconCheck /> Fatto!</> : <>Passo successivo →</>}
            </button>
          </div>
        </>
      )}

...
export default function RecipeDetailPage() {
  ...
  const { updateRecipe, removeRecipe, toggleStar, markCooked } = useRecipes();
  ...
  const handleToggleIngredient = useCallback((ingId: string) => {
    ...
    setIngredients(base.map((ing) => ing.id === ingId ? { ...ing, checked: !ing.checked } : ing));
  }, [ingredients, recipe]);

  const handleCheckAll = useCallback(() => {
    const base = ingredients ?? recipe?.ingredients ?? [];
    const all  = base.every((i) => i.checked);
    setIngredients(base.map((i) => ({ ...i, checked: !all })));
  }, [ingredients, recipe]);
      ...
      <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap", alignItems: "center", marginBottom: "0.875rem" }}>
        ...
        {recipe.source && (
          <a href={recipe.source} target="_blank" rel="noopener noreferrer"
            style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>Fonte ↗</a>
        )}
        {recipe.lastCooked && (
          <span style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
            Ultima volta: {new Date(recipe.lastCooked).toLocaleDateString("it-IT")}
          </span>
        )}
      </div>
